...
abstract sealed class Lucene99MemorySegmentByteVectorScorer
  ...
  static final class CosineScorer extends Lucene99MemorySegmentByteVectorScorer {
    ...
    public float score(int node) throws IOException {
      checkOrdinal(node);
      float raw = MemorySegmentVectorUtilSupport.INSTANCE.cosine(query, getSegment(node));
      return (1 + raw) / 2;
    }
  }
  ...
  static final class DotProductScorer extends Lucene99MemorySegmentByteVectorScorer {
    ...
    public float score(int node) throws IOException {
      ...
      float raw = MemorySegmentVectorUtilSupport.INSTANCE.dotProduct(query, getSegment(node));
  ...
  static final class EuclideanScorer extends Lucene99MemorySegmentByteVectorScorer {
    ...
    public float score(int node) throws IOException {
      checkOrdinal(node);
      float raw = MemorySegmentVectorUtilSupport.INSTANCE.squareDistance(query, getSegment(node));
      return 1 / (1f + raw);
    }
  }
