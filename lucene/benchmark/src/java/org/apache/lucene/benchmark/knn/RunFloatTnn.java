...
import org.apache.lucene.search.FaissFloatVectorSimilarityQuery;
...
public abstract class RunFloatTnn extends RecallTool.RunQueries<float[], Integer> {
  ...
  @CommandLine.Command(name = "run-float-tnn-faiss")
  public static class Faiss extends RunFloatTnn {
    @CommandLine.Option(
        names = {"-ef", "--efSearch"},
        description = "efSearch for radius based search",
        required = true)
    int efSearch;

    @Override
    public Query getQuery(float[] target) {
      return new FaissFloatVectorSimilarityQuery(knnField, target, resultSimilarity, efSearch);
    }
  }
